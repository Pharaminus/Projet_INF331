...
public class DataNutritionGeneration {
    public List<DataNutrition> generateRandomData(int count) {
        ...
        symptoms.add("nausées,maux_de_ventre");
        symptoms.add("toux");
    ...
    public void writeARFFFile(List<DataNutrition> dataList, int numFile) throws IOException {
        ...
        if (numFile == 1)
            path = path + "/data/suggestionCourData/dataNutritionForTrainning.arff";
        if (numFile == 0)
            path = path + "/data/suggestionCourData/dataNutritionprediction.arff";

        FileWriter writer = new FileWriter(path);

        writer.write("@relation MaladiesPredicion\n\n");
        writer.write("@attribute nomrepas1{poulet_rôti,légumes_sautés,riz_basmati,spaghetti_bolognaise,salade_verte,pain_à_l_ail,saumon_grillé,purée_de_pommes_de_terre,asperges_grillées}\n");
        writer.write("@attribute nomrepas2{poulet_rôti,légumes_sautés,riz_basmati,spaghetti_bolognaise,salade_verte,pain_à_l_ail,saumon_grillé,purée_de_pommes_de_terre,asperges_grillées}\n");
        writer.write("@attribute nomrepas3{poulet_rôti,légumes_sautés,riz_basmati,spaghetti_bolognaise,salade_verte,pain_à_l_ail,saumon_grillé,purée_de_pommes_de_terre,asperges_grillées}\n");
        writer.write("@attribute nommaladie{Grippe,Rhume,Sinusite,Gastro_entériteastro_entérite}\n");
        writer.write("@attribute syptoms{toux,eternuements,mal_de_gorge,fatigue,congestion_nasale,douleur_faciale,maux_de_tête,fièvre,diarrhée,vomissements,nausées,maux_de_ventre}\n\n");
        writer.write("@data\n");
